...
const Head = () => {
  ...
  return(
   <div className="grid grid-flow-col shadow-lg justify-between p-2 m-2 col-span-1">
    <div className="flex h-8 m-2 cursor-pointer">
    <img
  ...
  <div className='px-10 flex'>
    <div>
    <input className="px-5 w-1/2 border border-gray-400 p-2 rounded-l-full w-max " 
    type='text'
    value={searchQuerry}
    ...
    <div>
      <ul>
        {suggestions.map((s) =><li key={s} className='fixed bg-white w-[37rem] py-2 px-2'> {s} </li>)}
      </ul>
    </div>
    </div>
    <button className='border border-gray-400 px-5 py-2 rounded-r-full h-10 bg-gray-100'>Search</button>
  </div>
  <div className='flex h-8 mx-1 col-span-1'>
  <img
  alt='user-logo'
